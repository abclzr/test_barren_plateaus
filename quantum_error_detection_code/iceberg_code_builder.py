...
# cs = isotropicdepolarizingchannel(0.0, 1)
# cd = isotropicdepolarizingchannel(0.0, 2)
cs = isotropicdepolarizingchannel(0.0004, 1)
cd = isotropicdepolarizingchannel(0.003, 2)

class Iceberg_Code_Builder:
    def __init__(self, base_circuit: QuantumCircuit, t, b, qubit_list, a1, a2, clbit_allocator: ClassicalRegisterAllocator, syndrome_frequency=1, dmc=None):
        ...
        self.syndrome_noise = True
        self.post_selection_rate_list = []
        self.cnt = 0
        if dmc is not None:
            self.dmc = dmc
        else:
    ...
    def initialize(self):
        ...
        if self.dmc is not None:
            ...
            self.dmc.h(self.t)
            self.dmc.apply_general_kraus(cs, self.t)
            self.dmc.cnot(self.t, self.qubit_list[0])
            self.dmc.apply_general_kraus(cd, self.t, self.qubit_list[0])
            for i in range(0, len(self.qubit_list)-1):
                self.dmc.cnot(self.qubit_list[i], self.qubit_list[i+1])
                self.dmc.apply_general_kraus(cd, self.qubit_list[i], self.qubit_list[i+1])
            self.dmc.cnot(self.qubit_list[-1], self.b)
            self.dmc.apply_general_kraus(cd, self.qubit_list[-1], self.b)
            self.dmc.reset(self.a1)
            self.dmc.cnot(self.t, self.a1)
            self.dmc.apply_general_kraus(cd, self.t, self.a1)
            self.dmc.cnot(self.b, self.a1)
            self.dmc.apply_general_kraus(cd, self.b, self.a1)
    ...
    def logical_RZZ(self, theta: float, qubit1: int, qubit2: int):
        ...
        if self.dmc is not None:
            ...
            self.dmc.apply_general_kraus(cd, self.qubit_list[qubit1], self.qubit_list[qubit2])
            self.dmc_syndrome_measurement()
            self.refresh_dmc()
    ...
    def logical_RX(self, theta: float, qubit: int):
        ...
        if self.dmc is not None:
            ...
            self.dmc.apply_general_kraus(cd, self.t, self.qubit_list[qubit])
            self.dmc_syndrome_measurement()
            self.refresh_dmc()
    ...
    def logical_RZ(self, theta: float, qubit: int):
        ...
        if self.dmc is not None:
            ...
            self.dmc.apply_general_kraus(cd, self.b, self.qubit_list[qubit])
            self.dmc_syndrome_measurement()
            self.refresh_dmc()
    ...
    def logical_RZs(self, theta: float, qubit_indices: list[int]):
        ...
        if self.dmc is not None:
            for i in range(len(physical_qubits)-2):
                ...
                self.dmc.apply_general_kraus(cd, physical_qubits[i], physical_qubits[i+1])
            self.dmc.RZZ(physical_qubits[-2], physical_qubits[-1], theta=theta)
            self.dmc.apply_general_kraus(cd, physical_qubits[-2], physical_qubits[-1])
            for i in range(len(physical_qubits)-3, -1, -1):
                self.dmc.cnot(physical_qubits[i], physical_qubits[i+1])
                self.dmc.apply_general_kraus(cd, physical_qubits[i], physical_qubits[i+1])
            self.dmc_syndrome_measurement()
    ...
    def logical_X(self, qubit: int):
        ...
        if self.dmc is not None:
            ...
            self.dmc.apply_general_kraus(cs, self.qubit_list[qubit])
            self.dmc_syndrome_measurement()
            self.refresh_dmc()
    ...
    def post_select(self, qubit: int, flag=0):
        ...
        self.all_post_selection_rate *= p
        self.post_selection_rate_list.append(p)
    ...
    def dmc_syndrome_measurement(self):
        self.cnt += 1
        if self.cnt % self.syndrome_frequency != 0:
            return
        self.dmc.reset(self.a1)
        self.dmc.reset(self.a2)
        
        noise_free_syndrome = not self.syndrome_noise
        self.dmc.h(self.a2)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cs, self.a2)
        self.dmc.cnot(self.a2, self.t)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.a2, self.t)
        self.dmc.cnot(self.t, self.a1)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.t, self.a1)
        self.dmc.cnot(self.qubit_list[0], self.a1)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.qubit_list[0], self.a1)
        self.dmc.cnot(self.a2, self.qubit_list[0])
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.a2, self.qubit_list[0])
        
        for i in range(1, len(self.qubit_list)):
            self.dmc.cnot(self.a2, self.qubit_list[i])
            if not noise_free_syndrome:
                self.dmc.apply_general_kraus(cd, self.a2, self.qubit_list[i])
            self.dmc.cnot(self.qubit_list[i], self.a1)
            if not noise_free_syndrome:
                self.dmc.apply_general_kraus(cd, self.qubit_list[i], self.a1)
        
        self.dmc.cnot(self.b, self.a1)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.b, self.a1)
        self.dmc.cnot(self.a2, self.b)
        if not noise_free_syndrome:
            self.dmc.apply_general_kraus(cd, self.a2, self.b)
    ...
    def measurement(self) -> list[int]:
        ...
        if self.dmc is not None:
            noise_free_measurement = not self.syndrome_noise
            self.dmc.reset(self.a1)
            self.dmc.reset(self.a2)
            self.dmc.h(self.a1)
            if not noise_free_measurement:
                self.dmc.apply_general_kraus(cs, self.a1)
            
            self.dmc.cnot(self.a1, self.t)
            if not noise_free_measurement:
                self.dmc.apply_general_kraus(cd, self.a1, self.t)
            
            self.dmc.cnot(self.a1, self.a2)
            if not noise_free_measurement:
                self.dmc.apply_general_kraus(cd, self.a1, self.a2)
            for i in range(len(self.qubit_list)):
                self.dmc.cnot(self.a1, self.qubit_list[i])
                if not noise_free_measurement:
                    self.dmc.apply_general_kraus(cd, self.a1, self.qubit_list[i])
            self.dmc.cnot(self.a1, self.a2)
            if not noise_free_measurement:
                self.dmc.apply_general_kraus(cd, self.a1, self.a2)
            self.dmc.cnot(self.a1, self.b)
            if not noise_free_measurement:
                self.dmc.apply_general_kraus(cd, self.a1, self.b)
            ...
            self.dmc.apply_general_kraus([project_0, project_1], self.t)
            self.dmc.apply_general_kraus([project_0, project_1], self.b)
            for i in range(len(self.qubit_list)):
                self.dmc.apply_general_kraus([project_0, project_1], self.qubit_list[i])
    ...
    def copy_for_a_new_circuit(self, new_base_circuit: QuantumCircuit, new_clbit_allocator: ClassicalRegisterAllocator):
        ...
        new_builder.post_selection_rate_list = self.post_selection_rate_list
        new_builder.all_post_selection_rate = self.all_post_selection_rate
        return new_builder